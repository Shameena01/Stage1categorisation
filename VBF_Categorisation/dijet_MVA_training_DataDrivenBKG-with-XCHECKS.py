...
for i in clfs.keys():
    ...
    plt.title('2016 dijet VBF MVA score plot, full preselection, tight PUJID cuts')
    ...
    plt.legend()
   # plt.savefig('2016 dijet VBF MVA score plot with fullVBF pre-selection and tight PUJID cuts',bbox_inches = 'tight')
    plt.show()
...
def plot_rocs(rocs = {}, dump=False, range=[[0,1],[0,1]], label='', title='', rocs_train=None):
    ...
    plt.show()
plot_rocs(rocs_valid, label='2016-trained on datadriven-tested on data-driven', title = 'ROC for rejection of all backgrounds')
plot_rocs(rocs_valid_ggh, label='2016-trained on datadriven-tested on data-driven-xcheck2', title = 'ROC for ggh rejection')
plot_rocs(rocs_valid_dipho, label='2016-trained on datadriven-tested on data-driven-xcheck3', title = 'ROC for dipho rejection')
plot_rocs(rocs_valid_QCD, label='2016-trained on datadriven-tested on data-driven-xcheck4', title = 'ROC for QCD rejection')
def compare_train_test(clf,x_train,y_train,w_train,x_test,y_test,w_test, bins=100, label=''):
    ...
    plt.savefig('2016 dijet VBF overtraining x-check plot with fullVBF pre-selection and tight PUJID cuts',bbox_inches = 'tight')
    plt.show()
for i, c in clfs.items():
    print \
    compare_train_test(c,
                       X_train,Y_train,W_train, 
                       X_valid,Y_valid,W_valid, label='2016 dijet VBF overtraining x-check plot with fullVBF pre-selection and tight PUJID cuts')
