...
print(clfs.items())
print(Y_valid[I_valid=='ggh'])
print (W_valid[I_valid=='ggh'])
...
for i in clfs.keys():
    ...
    plt.title('2016 dijet MVA score plot-trained on MC background with relaxed pre-selection')
    plt.hist(tmva_train[(Y_train<0.5)], bins=50,
             weights=W_train[(Y_train<0.5)], 
             range=[-1,1],
             alpha=0.5, histtype='stepfilled', normed=1,label = 'SM')
    plt.hist(tmva_train[(Y_train>0.5)], bins=50,
             weights=W_train[(Y_train>0.5)], 
             range=[-1,1],
    ...
    plt.legend()
    plt.savefig('2016 dijet MVA score plot-trained on MC background with relaxed pre-selection.png', bbox_inches = 'tight')
    plt.show()
...
def plot_rocs(rocs = {}, dump=False, range=[[0,1],[0,1]], label='', title='', rocs_train=None):
    ...
    plt.show()
plot_rocs(rocs_valid, label='2016-trained on MC, tested on MC-xcheck1', title = 'ROC for all backgrounds')
plot_rocs(rocs_valid_ggh, label='2016-trained on MC, tested on MC-xcheck2', title = 'ROC for ggh rejection')
plot_rocs(rocs_valid_dipho, label='2016-trained on MC, tested on MC-xcheck3', title = 'ROC for dipho rejection')
plot_rocs(rocs_valid_qcd, label='2016-trained on MC, tested on MC-xcheck4', title = 'ROC for qcd rejection')
plot_rocs(rocs_valid_gjet, label='2016-trained on MC, tested on MC-xcheck5', title = 'ROC for gjet rejection')
def compare_train_test(clf,x_train,y_train,w_train,x_test,y_test,w_test, bins=100, label=''):
    ...
    plt.savefig('2016 dijet MVA overtraining xcheck plot-trained on MC background with relaxed pre-selection.png')
    plt.show()
for i, c in clfs.items():
    print \
    compare_train_test(c,
                       X_train,Y_train,W_train, 
                       X_valid,Y_valid,W_valid, label='2016 dijet MVA overtraining xcheck plot-trained on MC background with relaxed pre-selection')
