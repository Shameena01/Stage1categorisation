...
for i in clfs.keys():
    ...
    plt.title('2016 dijet VBF MVA score plot, full VBF preselection, trained on mass side-band backgrounds')
    plt.hist(tmva_train[(Y_train<0.5)], bins=50,
             weights=W_train[(Y_train<0.5)], 
             range=[-1,1],
             alpha=0.5, histtype='stepfilled', normed=1, label = 'Mass sideband background')
    plt.hist(tmva_train[(Y_train>0.5)], bins=50,
             weights=W_train[(Y_train>0.5)], 
             range=[-1,1],
    ...
    plt.savefig('2016_dijet_VBF_MVA_score_plot_full_VBF_preselection_trained_on_mass_ side-band_backgrounds.png', bbox_inches = 'tight')
    plt.show()
...
def plot_rocs(rocs = {}, dump=False, range=[[0,1],[0,1]], label='', title='', rocs_train=None):
    ...
    plt.show()
plot_rocs(rocs_valid, label='2016_ROC_XCHECK1_full_VBF_preselection_trained_on_MSB_tested_on_datadriven', title = 'ROC for all backgrounds')
plot_rocs(rocs_valid_ggh, label='2016_ROC_XCHECK2_full_VBF_preselection_trained_on_MSB_tested_on_datadriven', title = 'ROC for ggh rejection')
plot_rocs(rocs_valid_dipho, label='2016_ROC_XCHECK3_full_VBF_preselection_trained_on_MSB_tested_on_datadriven', title = 'ROC for dipho rejection')
plot_rocs(rocs_valid_QCD, label='2016_ROC_XCHECK4_full_VBF_preselection_trained_on_MSB_tested_on_datadriven', title = 'ROC for QCD rejection')
def compare_train_test(clf,x_train,y_train,w_train,x_test,y_test,w_test, bins=100, label=''):
    fig = plt.figure(figsize=(5,5))
    plt.title('2016 dijet VBF MVA overtraining xcheck, full VBF preselection, trained on mass side-band backgrounds')
    decisions = []
    weight    = []
    print clf
    ...
    plt.savefig('2016_dijet_VBF_MVA_overtrain_xcheck_full_VBF_preselection_trained_on_mass_ side-band_backgrounds.png')
    plt.show()
for i, c in clfs.items():
    print \
