...
for i in clfs.keys():
    ...
    plt.title('2016_relax_preselection_mva score plot_trained_on_ggh_and_dipho_background_only')
    plt.hist(tmva_train[(Y_train<0.5)], bins=50,
             weights=W_train[(Y_train<0.5)], 
             range=[-1,1],
             alpha=0.5, histtype='stepfilled', normed=1, label = 'ggh and dipho background')
    plt.hist(tmva_train[(Y_train>0.5)], bins=50,
             weights=W_train[(Y_train>0.5)], 
             range=[-1,1],
    ...
    plt.savefig('2016_relax_preselection_mva score plot_trained_on_ggh_and_dipho_background_only.png', bbox_inches = 'tight')
    plt.show()
...
def plot_rocs(rocs = {}, dump=False, range=[[0,1],[0,1]], label='', title='', rocs_train=None):
    ...
    plt.show()
plot_rocs(rocs_valid, label='2016_ROC_XCHECK1_relax_VBF_preselection_trained_on_ggh_and_dipho_tested_on_mc', title = 'ROC for all backgrounds')
plot_rocs(rocs_valid_ggh, label='2016_ROC_XCHECK2_relax_VBF_preselection_trained_on_ggh_and_dipho_tested_on_mc', title = 'ROC for ggh rejection')
plot_rocs(rocs_valid_dipho, label='2016_ROC_XCHECK3_relax_VBF_preselection_trained_on_ggh_and_dipho_tested_on_mc', title = 'ROC for dipho rejection')
plot_rocs(rocs_valid_qcd, label='2016_ROC_XCHECK4_relax_VBF_preselection_trained_on_ggh_and_dipho_tested_on_mc', title = 'ROC for jet-jet rejection')
plot_rocs(rocs_valid_gjet, label='2016_ROC_XCHECK5_relax_VBF_preselection_trained_on_ggh_and_dipho_tested_on_mc', title = 'ROC for gjet rejection')
def compare_train_test(clf,x_train,y_train,w_train,x_test,y_test,w_test, bins=100, label=''):
    ...
    plt.savefig('2016_relax_preselection_overtrain_xcheck_trained_on_ggh_and_dipho_background_only.png')
    plt.show()
for i, c in clfs.items():
    print \
    compare_train_test(c,
                       X_train,Y_train,W_train, 
                       X_valid,Y_valid,W_valid, label='2016_relax_preselection_overtrain_xcheck_trained_on_ggh_and_dipho_background_only')
