...
allVars = ['dipho_leadIDMVA','dipho_subleadIDMVA','dipho_lead_ptoM','dipho_sublead_ptoM',
              ...
              'dipho_cosphi','vtxprob','sigmarv','sigmawv','HTXSstage1cat','dipho_mass','weight','dijet_Zep','dijet_LeadJPt','dijet_SubJPt','dijet_abs_dEta','dijet_Mjj','dijet_dphi','dijet_minDRJetPho','dijet_dipho_dphi_trunc','dipho_djet_ptHjj']
...
trainTotal = pd.read_hdf('/eos/user/s/sbonomal/examples_mine/ThreeClass_with_DataDriven_2018.h5')

print np.unique(trainTotal['sample'])
...
ggH_df = trainTotal[trainTotal['sample']=='ggh']
vbf_df = trainTotal[trainTotal['sample']=='vbf']
dipho_df = trainTotal[trainTotal['sample']=='dipho']
QCD_df = trainTotal[trainTotal['sample']=='QCD']
...
print 'dipho events'
print dipho_df.shape[0]
print 'QCD events'
print QCD_df.shape[0]

#selecting only the processes we need

trainTotal = trainTotal[(trainTotal['sample']=='ggh')|(trainTotal['sample']=='vbf')|(trainTotal['sample']=='dipho')|(trainTotal['sample']=='QCD')]
...
trainTotal  = trainTotal[(trainTotal['sample']=='QCD')|(trainTotal['dipho_leadIDMVA']>-0.2)]
...
trainTotal  = trainTotal[(trainTotal['sample']=='QCD')|(trainTotal['dipho_subleadIDMVA']>-0.2)]
...
trainTotal['weightR'] = trainTotal['weight']# now that we use datadriven, we do not downweight QCD anymore

print 'before applying functions'
print np.unique(trainTotal['sample'])
...
def plot_variable(var='cosThetaStar', var_label = '$\cos\,\theta^*$', setrange=(-1,1)):
 ...
 # numpy_Data = df_Data[var].values
  ...
  plt.savefig('ThreeClass_var_plots_2018/%s.png'%var,bbox_inches = 'tight')
  plt.savefig('ThreeClass_var_plots_2018/%s.pdf'%var,bbox_inches = 'tight')
